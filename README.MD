...
**RHey** is an advanced JavaScript/TypeScript array utility library that extends the native `Array` class with powerful methods for manipulating and querying arrays, especially arrays of objects. RHey combines the convenience of common array operations with the flexibility of custom index handling and condition-based operations, making it a valuable tool for any JavaScript developer.
...
- **`center`**: Returns the center element(s) in the array.
...
console.log(arr1.differenceBy(arr2, "id")); // Assumes arrays of objects with 'id' property
```
...
// Flatten
const nestedArr = new RHey([1, 2], [3, 4], 5);
console.log(nestedArr.flatten()); // [1, 2, 3, 4, 5]

// Unique
const duplicateArr = new RHey(1, 2, 2, 3, 4, 4, 5);
console.log(duplicateArr.unique()); // [1, 2, 3, 4, 5]
